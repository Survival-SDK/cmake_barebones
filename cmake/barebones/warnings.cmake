...
function(bb_add_more_warnings_c_all COMPILE_OPTIONS)
    bb_add_more_warnings_c_alloc(${COMPILE_OPTIONS})
    bb_add_more_warnings_c_array(${COMPILE_OPTIONS})
    bb_add_more_warnings_c_asciiz(${COMPILE_OPTIONS})
    bb_add_more_warnings_c_basic(${COMPILE_OPTIONS})
    bb_add_more_warnings_c_enum(${COMPILE_OPTIONS})
    bb_add_more_warnings_c_format(${COMPILE_OPTIONS})
    bb_add_more_warnings_c_float(${COMPILE_OPTIONS})
    bb_add_more_warnings_c_preprocessor(${COMPILE_OPTIONS})
    bb_add_more_warnings_c_simd(${COMPILE_OPTIONS})
    bb_add_more_warnings_c_switch(${COMPILE_OPTIONS})
    bb_add_more_warnings_c_threads(${COMPILE_OPTIONS})
endfunction()

function(bb_add_more_warnings_cxx_all COMPILE_OPTIONS)
    bb_add_more_warnings_cxx_alloc(${COMPILE_OPTIONS})
    bb_add_more_warnings_cxx_array(${COMPILE_OPTIONS})
    bb_add_more_warnings_cxx_asciiz(${COMPILE_OPTIONS})
    bb_add_more_warnings_cxx_basic(${COMPILE_OPTIONS})
    bb_add_more_warnings_cxx_enum(${COMPILE_OPTIONS})
    bb_add_more_warnings_cxx_format(${COMPILE_OPTIONS})
    bb_add_more_warnings_cxx_float(${COMPILE_OPTIONS})
    bb_add_more_warnings_cxx_preprocessor(${COMPILE_OPTIONS})
    bb_add_more_warnings_cxx_simd(${COMPILE_OPTIONS})
    bb_add_more_warnings_cxx_switch(${COMPILE_OPTIONS})
    bb_add_more_warnings_cxx_threads(${COMPILE_OPTIONS})
    bb_add_more_warnings_cxx_template(${COMPILE_OPTIONS})
    bb_add_more_warnings_cxx_virtual(${COMPILE_OPTIONS})
endfunction()

function(bb_add_more_warnings_c CATEGORY OPTIONS)
    if (${CATEGORY} STREQUAL all)
        bb_add_more_warnings_c_all(${OPTIONS})
    elseif (${CATEGORY} STREQUAL alloc)
        bb_add_more_warnings_c_alloc(${OPTIONS})
    elseif (${CATEGORY} STREQUAL array)
        bb_add_more_warnings_c_array(${OPTIONS})
    elseif (${CATEGORY} STREQUAL asciiz)
        bb_add_more_warnings_c_asciiz(${OPTIONS})
    elseif (${CATEGORY} STREQUAL basic)
        bb_add_more_warnings_c_basic(${OPTIONS})
    elseif (${CATEGORY} STREQUAL c89)
        bb_add_more_warnings_c_c89(${OPTIONS})
    elseif (${CATEGORY} STREQUAL cuda)
        bb_add_more_warnings_c_cuda(${OPTIONS})
    elseif (${CATEGORY} STREQUAL documentation)
        bb_add_more_warnings_c_documentation(${OPTIONS})
    elseif (${CATEGORY} STREQUAL enum)
        bb_add_more_warnings_c_enum(${OPTIONS})
    elseif (${CATEGORY} STREQUAL format)
        bb_add_more_warnings_c_format(${OPTIONS})
    elseif (${CATEGORY} STREQUAL float)
        bb_add_more_warnings_c_float(${OPTIONS})
    elseif (${CATEGORY} STREQUAL opencl)
        bb_add_more_warnings_c_opencl(${OPTIONS})
    elseif (${CATEGORY} STREQUAL openmp)
        bb_add_more_warnings_c_openmp(${OPTIONS})
    elseif (${CATEGORY} STREQUAL preprocessor)
        bb_add_more_warnings_c_preprocessor(${OPTIONS})
    elseif (${CATEGORY} STREQUAL simd)
        bb_add_more_warnings_c_simd(${OPTIONS})
    elseif (${CATEGORY} STREQUAL spirv)
        bb_add_more_warnings_c_spirv(${OPTIONS})
    elseif (${CATEGORY} STREQUAL switch)
        bb_add_more_warnings_c_switch(${OPTIONS})
    elseif (${CATEGORY} STREQUAL threads)
        bb_add_more_warnings_c_threads(${OPTIONS})
    else()
        message(WARNING "Unknown warning category for C language - \"${CATEGORY}\"")
    endif()
endfunction()

function(bb_add_more_warnings_cxx CATEGORY OPTIONS)
    if (${CATEGORY} STREQUAL all)
        bb_add_more_warnings_cxx_all(${OPTIONS})
    elseif (${CATEGORY} STREQUAL alloc)
        bb_add_more_warnings_cxx_alloc(${OPTIONS})
    elseif (${CATEGORY} STREQUAL array)
        bb_add_more_warnings_cxx_array(${OPTIONS})
    elseif (${CATEGORY} STREQUAL asciiz)
        bb_add_more_warnings_cxx_asciiz(${OPTIONS})
    elseif (${CATEGORY} STREQUAL basic)
        bb_add_more_warnings_cxx_basic(${OPTIONS})
    elseif (${CATEGORY} STREQUAL cuda)
        bb_add_more_warnings_cxx_cuda(${OPTIONS})
    elseif (${CATEGORY} STREQUAL documentation)
        bb_add_more_warnings_cxx_documentation(${OPTIONS})
    elseif (${CATEGORY} STREQUAL enum)
        bb_add_more_warnings_cxx_enum(${OPTIONS})
    elseif (${CATEGORY} STREQUAL format)
        bb_add_more_warnings_cxx_format(${OPTIONS})
    elseif (${CATEGORY} STREQUAL float)
        bb_add_more_warnings_cxx_float(${OPTIONS})
    elseif (${CATEGORY} STREQUAL opencl)
        bb_add_more_warnings_cxx_opencl(${OPTIONS})
    elseif (${CATEGORY} STREQUAL openmp)
        bb_add_more_warnings_cxx_openmp(${OPTIONS})
    elseif (${CATEGORY} STREQUAL preprocessor)
        bb_add_more_warnings_cxx_preprocessor(${OPTIONS})
    elseif (${CATEGORY} STREQUAL simd)
        bb_add_more_warnings_cxx_simd(${OPTIONS})
    elseif (${CATEGORY} STREQUAL spirv)
        bb_add_more_warnings_cxx_spirv(${OPTIONS})
    elseif (${CATEGORY} STREQUAL switch)
        bb_add_more_warnings_cxx_switch(${OPTIONS})
    elseif (${CATEGORY} STREQUAL template)
        bb_add_more_warnings_cxx_template(${OPTIONS})
    elseif (${CATEGORY} STREQUAL threads)
        bb_add_more_warnings_cxx_threads(${OPTIONS})
    elseif (${CATEGORY} STREQUAL virtual)
        bb_add_more_warnings_cxx_virtual(${OPTIONS})
    else()
        message(WARNING "Unknown warning category for C++ language - \"${CATEGORY}\"")
    endif()
endfunction()
...
function(bb_add_more_warnings)
    ...
    if (${_BB_ADD_MORE_WARNINGS_LANG} STREQUAL C)
        foreach(_CATEGORY IN LISTS _BB_ADD_MORE_WARNINGS_CATEGORIES)
            bb_add_more_warnings_c(${_CATEGORY}
                ${_BB_ADD_MORE_WARNINGS_OPTIONS})
        endforeach()
    endif()
    if (LANG STREQUAL C++)
        foreach(_CATEGORY IN LISTS _BB_ADD_MORE_WARNINGS_CATEGORIES)
            bb_add_more_warnings_cxx(${_CATEGORY}
                ${_BB_ADD_MORE_WARNINGS_OPTIONS})
        endforeach()
    endif()
